...
namespace DataStructures
{
    ...
    {
        private readonly Dictionary<T, List<object>> _adjacencyList = new Dictionary<T, List<object>>();
        ...
        {
            if (!_adjacencyList.TryAdd(key, new List<object>()))
        ...
        internal void AddEdge(T vertex1, T vertex2, int? weight = null)
        {
            ...
            if (!_adjacencyList.TryGetValue(vertex2, out List<object>? vertex2Array))
        ...
        internal void RemoveVertex(T vertex)
        {
            ...
            foreach (List<object> value in _adjacencyList.Values)
            {
                value.Remove(vertex);
            }
        ...
        private void Dfs(T vertex, Dictionary<T, bool> visited, List<T> results)
        {
            ...
            results.Add(vertex);
            List<object>? values = _adjacencyList.GetValueOrDefault(vertex);
        ...
        internal List<T> DepthFirstIterative(T vertex)
        {
            ...
            while (stack.Length() > 0)
            {
                ...
                List<object>? neighbors = _adjacencyList.GetValueOrDefault(popped);
        ...
        internal List<T> BreadthFirstTraversal(T vertex)
        {
            ...
            while (queue.Length() > 0)
            {
                ...
                List<object>? neighbors = _adjacencyList.GetValueOrDefault(currentVertex);
